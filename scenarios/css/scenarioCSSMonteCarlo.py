...
def runMonteCarlo():
    ...
    mcController.addDispersion(
        NormalVectorCartDispersion(
            dispCSS1Orientation, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]
        )
    )
    mcController.addDispersion(
        UniformDispersion(
            dispCSS1Fov,
            [(80.0 - 1.0 * 80.0) * macros.D2R, (80.0 + 1.0 * 80.0) * macros.D2R],
        )
    )
    mcController.addDispersion(
        NormalVectorCartDispersion(dispCSS1Position, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0])
    )
    mcController.addDispersion(UniformDispersion(dispCSS1KellyFactor, [0.0, 1.0]))
    if MULTIPLE_INSTRUMENT:
        mcController.addDispersion(
            NormalVectorCartDispersion(
                dispCSS2Orientation, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]
            )
        )
        mcController.addDispersion(
            UniformDispersion(
        ...
        mcController.addDispersion(
            NormalVectorCartDispersion(
                dispCSS3Orientation, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]
            )
        )
        mcController.addDispersion(
            UniformDispersion(
        ...
        mcController.addDispersion(
            NormalVectorCartDispersion(
                dispCSS2Position, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]
            )
        )
        mcController.addDispersion(
            NormalVectorCartDispersion(
                dispCSS3Position, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]
            )
        )
...
def createScenarioCSS():
    ...
    if MULTIPLE_PLANET:
        ...
        simulationTime = macros.sec2nano(500.0)
    ...
    if MULTIPLE_INSTRUMENT:
        ...
        CSS2.ModelTag = "CSS2"
        setupCSS(CSS2)
        CSS2.nHat_B = np.array([-1.0, 0.0, 0.0])
        ...
        CSS3.ModelTag = "CSS3"
        setupCSS(CSS3)
        CSS3.nHat_B = np.array([0.0, -1.0, 0.0])
    ...
    scSim.msgRecList[albDataOutMsgName[0]] = albModule.albOutMsgs[0].recorder(
        samplingTime
    )
    scSim.AddModelToTask(simTaskName, scSim.msgRecList[albDataOutMsgName[0]])
    if MULTIPLE_INSTRUMENT:
        scSim.msgRecList[cssDataOutMsgName[1]] = CSS2.cssDataOutMsg.recorder(
            samplingTime
        )
        scSim.AddModelToTask(simTaskName, scSim.msgRecList[cssDataOutMsgName[1]])
        scSim.msgRecList[cssDataOutMsgName[2]] = CSS3.cssDataOutMsg.recorder(
            samplingTime
        )
        scSim.AddModelToTask(simTaskName, scSim.msgRecList[cssDataOutMsgName[2]])
        scSim.msgRecList[albDataOutMsgName[1]] = albModule.albOutMsgs[1].recorder(
            samplingTime
        )
        scSim.AddModelToTask(simTaskName, scSim.msgRecList[albDataOutMsgName[1]])
        scSim.msgRecList[albDataOutMsgName[2]] = albModule.albOutMsgs[2].recorder(
            samplingTime
        )
...
def plotSim(data, retentionPolicy):
    ...
    if MULTIPLE_INSTRUMENT:
        ...
        dataAlb[:, 1] = data["messages"][albDataOutMsgName[1] + ".albedoAtInstrument"][
            :, 1
        ]
        dataAlb[:, 2] = data["messages"][albDataOutMsgName[2] + ".albedoAtInstrument"][
            :, 1
        ]
    ...
    pltName = (
        ...
        + str(1)
        + str(int(ALBEDO_DATA))
        + str(int(MULTIPLE_INSTRUMENT))
        + str(int(MULTIPLE_PLANET))
    )
